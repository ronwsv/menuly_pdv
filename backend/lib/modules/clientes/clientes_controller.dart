...
class ClientesController {
  ...
  Future<Response> obterPorId(Request request) async {
    ...
    final cliente = await _service.obterPorId(id);

    return JsonResponse.ok(cliente);
  }

  Future<Response> criar(Request request) async {
    final body = await parseBody(request);
    final cliente = await _service.criar(body);

    return JsonResponse.created(cliente);
  }

  Future<Response> atualizar(Request request) async {
    final id = int.parse(request.params['id']!);
    final body = await parseBody(request);
    final cliente = await _service.atualizar(id, body);
  ...
  Future<Response> excluir(Request request) async {
    ...
    return JsonResponse.ok({'message': 'Cliente excluído com sucesso'});
  }
}

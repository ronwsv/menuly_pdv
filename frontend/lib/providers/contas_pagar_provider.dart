...
class ContasPagarProvider extends ChangeNotifier {
  ...
  ContasPagarProvider(this._api);

  List<ContaPagar> get contas => _contas;
  ...
  Future<void> carregarContas({int page = 1}) async {
    ...
    try {
      ...
      final result =
          await _api.get(ApiConfig.contasPagar, queryParams: params);
  ...
  Future<void> carregarTotais() async {
    try {
      final result = await _api.get(ApiConfig.contasPagarTotais);
  ...
  Future<void> criarConta(Map<String, dynamic> data) async {
    await _api.post(ApiConfig.contasPagar, body: data);
    await carregarContas();
    await carregarTotais();
  }

  Future<void> atualizarConta(int id, Map<String, dynamic> data) async {
    await _api.put(ApiConfig.contaPagarById(id), body: data);
  ...
  Future<void> darBaixa(
    ...
    await _api.post(ApiConfig.contaPagarBaixa(id), body: {
  ...
  Future<void> cancelarConta(int id) async {
    await _api.post(ApiConfig.contaPagarCancelar(id), body: {});
    await carregarContas(page: _page);
    await carregarTotais();
  }

  Future<void> excluirConta(int id) async {
    await _api.delete(ApiConfig.contaPagarById(id));
    await carregarContas(page: _page);
    await carregarTotais();
  }
}

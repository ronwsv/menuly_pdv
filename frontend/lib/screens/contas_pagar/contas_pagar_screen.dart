...
class _ContasPagarScreenState extends State<ContasPagarScreen> {
  ...
  void initState() {
    super.initState();
    final provider = context.read<ContasPagarProvider>();
  ...
  void _abrirFormulario([ContaPagar? conta]) async {
    final result = await showDialog<bool>(
      context: context,
      builder: (_) => ChangeNotifierProvider.value(
        value: context.read<ContasPagarProvider>(),
        child: _ContaFormDialog(conta: conta),
      ),
    );
    if (result == true && mounted) {
      context.read<ContasPagarProvider>().carregarContas();
      context.read<ContasPagarProvider>().carregarTotais();
    }
  }

  void _abrirBaixa(ContaPagar conta) async {
    final result = await showDialog<bool>(
      context: context,
      builder: (_) => ChangeNotifierProvider.value(
        value: context.read<ContasPagarProvider>(),
  ...
  void _confirmarCancelamento(ContaPagar conta) async {
    ...
    if (confirm == true && mounted) {
      try {
        await context.read<ContasPagarProvider>().cancelarConta(conta.id);
        if (mounted) {
  ...
  void _confirmarExclusao(ContaPagar conta) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        backgroundColor: AppTheme.cardSurface,
    ...
    if (confirm == true && mounted) {
      try {
        await context.read<ContasPagarProvider>().excluirConta(conta.id);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Consumer<ContasPagarProvider>(
        ...
        SizedBox(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Buscar por descricao ou fornecedor...',
              prefixIcon:
  ...
  Widget _buildStatCards(ContasPagarProvider provider) {
  ...
  Widget _buildFilterChips(ContasPagarProvider provider) {
  ...
  Widget _buildBody(ContasPagarProvider provider) {
  ...
  DataRow _buildRow(ContaPagar conta) {
    ...
    return DataRow(cells: [
      ...
      DataCell(Text(
        conta.dataPagamento != null
            ? '${dateFormat.format(conta.dataPagamento!)}\n${conta.formaPagamento ?? ''}'
            : '-',
        style: TextStyle(color: AppTheme.textSecondary, fontSize: 13),
      )),
  ...
  Widget _buildActions(ContaPagar conta) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        if (conta.isPendente) ...[
...
class _ContaFormDialogState extends State<_ContaFormDialog> {
  ...
  late final TextEditingController _fornecedorIdCtrl;
  late final TextEditingController _compraIdCtrl;
  ...
  void initState() {
    ...
    _fornecedorIdCtrl = TextEditingController(
        text: c?.fornecedorId != null ? c!.fornecedorId.toString() : '');
    _compraIdCtrl = TextEditingController(
        text: c?.compraId != null ? c!.compraId.toString() : '');
    if (c != null) {
  ...
  void dispose() {
    ...
    _fornecedorIdCtrl.dispose();
    _compraIdCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _salvar() async {
    ...
    if (_fornecedorIdCtrl.text.trim().isNotEmpty) {
      data['fornecedor_id'] = int.tryParse(_fornecedorIdCtrl.text.trim());
    }
    if (_compraIdCtrl.text.trim().isNotEmpty) {
      data['compra_id'] = int.tryParse(_compraIdCtrl.text.trim());
    }
          ...
          child: Padding(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    _isEditing ? 'Editar Conta' : 'Nova Conta a Pagar',
                    style: TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.w600,
                      color: AppTheme.textPrimary,
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'Tipo',
                      hintText: 'Ex: boleto, nota fiscal, aluguel',
                    ),
                    style: TextStyle(
                        color: AppTheme.textPrimary, fontSize: 14),
                  ),
                  ...
                  // Fornecedor ID + Compra ID
                  Row(
                    children: [
                      Expanded(
                        child: TextFormField(
                          controller: _fornecedorIdCtrl,
                          decoration: InputDecoration(
                      ...
                      Expanded(
                        child: TextFormField(
                          controller: _compraIdCtrl,
                          decoration:
                              InputDecoration(labelText: 'ID Compra'),
                          style: TextStyle(
                              color: AppTheme.textPrimary, fontSize: 14),
                          keyboardType: TextInputType.number,
                          inputFormatters: [
...
class _BaixaDialogState extends State<_BaixaDialog> {
  ...
  Future<void> _confirmarBaixa() async {
    ...
    try {
      await context.read<ContasPagarProvider>().darBaixa(
            widget.conta.id,
            _formaPagamento,
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (widget.conta.fornecedorNome != null) ...[
                      SizedBox(height: 4),
                      Text(
                        'Fornecedor: ${widget.conta.fornecedorNome}',
                        style: TextStyle(
                          color: AppTheme.textSecondary,
                          fontSize: 13,
                        ),
